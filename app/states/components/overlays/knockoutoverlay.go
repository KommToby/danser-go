...
func (overlay *KnockoutOverlay) DrawHUD(batch *batch.QuadBatch, colors []color2.Color, alpha float64) {
	...
	for _, r := range replays {
		...
		if r.Mods != "" {
			if settings.Knockout.ModMults {
				...
				pWidth += overlay.font.GetWidth(scl*0.8, "+"+r.Mods+" ("+strconv.FormatFloat(r.ModsV.GetScoreMultiplier(), 'f', 2, 64)+"x)")
			} else {
	...
	for _, rep := range overlay.playersArray {
		...
		if r.Mods != "" {
			if settings.Knockout.ModMults {
				//                                                       f is decimal floating-point, -1 is default, 64 is the bit size
				width += overlay.font.GetWidth(scl*0.8, "+"+r.Mods+" ("+strconv.FormatFloat(r.ModsV.GetScoreMultiplier(), 'f', 2, 64)+"x)")
			} else {
	...
	for _, rep := range overlay.playersArray {
		...
		if r.Mods != "" {
			if settings.Knockout.ModMults {
				overlay.font.DrawOrigin(batch, 3.2*scl+width+nWidth+xSlideLeft, rowBaseY+ascScl, vector.BottomLeft, scl*0.8, false, "+"+r.Mods+" ("+strconv.FormatFloat(r.ModsV.GetScoreMultiplier(), 'f', 2, 64)+"x)")
			} else {
